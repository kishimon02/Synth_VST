...
出力ガード (0 dBFS 以下は素通し・それ以上は頭打ち・NaN/Inf でブロックを無音化)、
プリセットのパラメータ ID 妥当性・カテゴリ・保存/読込、エンジン全体のレンダリング (有限・不連続なし・解放後に無音) を検証する。

最後の「Stability soak」は、ブロックサイズ・テンポ・ノート・パラメータをランダムに変えながら
アルペジエーター → ボイス → FX → 出力ガードを通しで回し、全サンプルが有限で頭打ちの範囲内に収まることを見る。
既定は 30 秒分。リリース前は 30 分分を回す (実時間では 2 分ほど):
...
- Phase 6 (仕上げ) 完了: 出力ガード (NaN/Inf の遮断とソフトクリップ)、Panic、dsp 負荷表示、
  壊れた状態ツリーの読み飛ばし、30 分ぶんの soak テスト (v1.0.0)
...
長時間の演奏で「気づいたら音が壊れていた」を防ぐための仕掛け。

- **出力ガード** (常時、切れない): ホストに渡す直前で
  - NaN / Inf が 1 サンプルでも出たら、そのブロックを無音にしてボイス・フィルター・アルペジオ・FX の状態を
    まとめてリセットする。ディレイやレゾナンスのフィードバックに入った異常値がその後ずっと鳴り続けるのを止める
  - 0 dBFS を超えた分だけカーブで頭打ちにする (+6 dB で止まる)。**0 dBFS 以下は 1 ビットも変えない**ので、
    普段の音は一切変わらない
- **Panic** (画面右上): 鳴りっぱなしの音・アルペジオの保持ノート・AI の試聴を全部止めて、FX の残響も消す。
  DAW が note off を取りこぼしたときの復帰用
- **状態表示** (画面上部中央): `voices 3    120.0 BPM    dsp 4%`
  - `dsp` はオーディオコールバックの持ち時間に対する処理時間の割合 (ホストの CPU メーターとは別物)
  - 出力ガードが働くと `!1` のように回数が出る。通常は出ない

## Studio One の中で使うときの注意

DAW の中ではホストがキーボードとマウスを先に受け取るので、次の 2 つに癖がある (実機で確認済み)。

- **文字の貼り付け**: Studio One は **Ctrl+V を自分のショートカットとして取ってしまう**ので、プラグインには届かない。
  - AI タブ左下の **Paste** ボタン、または入力欄を**右クリック → Paste** を使う
  - Settings の API キー欄にも **Paste** ボタンがある。キーは空白と改行を取り除いて保存するので、
    コピー時に改行が混ざっても 401 にならない
  - 文字そのもの (IME 入力を含む) は普通に打てる。効かないのは Ctrl+V などホスト側のショートカットだけ
- **パートの取り込み**: Studio One の「コピー」は **DAW 内部の形式**でクリップボードに入るため、プラグイン側では読めない
  (実機で確認済み。クリップボードには Studio One 専用のデータしか載らない)。取り込みは次の 3 通り:
  1. **Rec**: 録音ボタンを押して再生 / 演奏する (このトラックに来る MIDI)
  2. **パートを AI タブへドラッグ**: アレンジのパートをそのままプラグイン画面へドラッグ&ドロップ。
     受け付ける状態になると画面が緑枠になる
  3. **Add .mid...**: パートをデスクトップにドラッグするか書き出して、ファイルとして読み込む (ホスト内でも動作確認済み)
- **Drag to DAW**: `.mid` のドラッグ&ドロップは Studio One 4 で動く。ただし**プラグイン画面が重なっている場所には落とせない**。
  - プラグイン画面を少し脇にどけて、アレンジのトラックが見えている所までドラッグする
  - 画面を動かしたくないときは **Save .mid...** で保存して、ブラウザやエクスプローラーから読み込む
- VST3 を作り直すときは **Studio One を終了してからビルド**する (起動中は DLL が置き換えられない)
